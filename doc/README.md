...
- 無精確整詞的逐字區段可補足省略的聲調；音節及明確聲調必須相符，以補調成本與低頻不確定性避免強迫合成少見詞。組字與候選共用補救入口，原始讀音、按鍵及人工選字保留。詳見 [省略聲調整詞補救](省略聲調整詞補救_20260921.md)。
- 候選清單先確定順序，再依同一索引產生預覽；第零項代表目前正文。
- 候選清單未開啟時，游標移動只改變焦點，不能單憑另一份首選排序覆蓋正文；清單開啟時，方向鍵與 Home／End 先確認目前候選。
- 確認候選後，未與鎖定範圍交錯的完整詞保留整句上下文。`confirmSegments` 共用替換交疊確認範圍的規則，避免舊整詞覆蓋新單字。
- 確認並前進時，靠右模式移到下一詞起點，靠左及雙側模式移到下一詞終點，使焦點均落在下一詞。
...
`rebaseOverrides` 在插入、刪除或跨語言重切時同步更新來源按鍵與前後鎖定位置，移除與編輯範圍交錯的舊鎖定。候選帶有 `inputSpan` 時，確認前會比對目前來源；已失效的範圍不套用。預覽不提交文字，也不建立人工確認。詞內尚未完成的注音僅在呈現時分開跨越插入點的詞段，原讀音座標、來源及確認鎖定保留；原注音依音節切分，一般候選依完整字元切分。
...
英文精確詞尾會向後檢查最多四個原始按鍵；遇到明確聲調時，沿用注音解析及詞庫判斷後方是否為完整有效音節，避免較長英文候選吃掉中文首鍵。中文區段的右邊界若仍有第一聲音節，使用完成音節的狀態副本評估整詞，不改動逐鍵輸入狀態。
...
| 紀錄 | 範圍 |
| --- | --- |
| [省略聲調整詞補救](省略聲調整詞補救_20260921.md) | 「區消」問題、完整詞召回、低頻誤改防護與驗證 |
| [五輪組字選字修正](組字選字五輪檢查與修正_20260921.md) | 游標前進、重選鎖定、詞頻保存、快取隔離與詞內插入 |
| [組字選字策略修正](組字選字策略檢查與修正_20260921.md) | 第一聲末音節、完整左文、批次評分及分詞查詢 |
| [混打邊界修正](混打邊界修正_20260921.md) | 英文詞尾與中文首鍵的邊界 |
| [全專案修正驗證](專案修正驗證_20260921.md) | 提交與長輸入、模型安裝、資料契約、CLI 與備份 |
| [原始深度檢查](專案深度檢查_20260920.md) | 保留修正前問題及當時驗證結果 |

在專案根目錄執行隔離 Smoke，不安裝或切換正式輸入法：

```sh
python3 src/unifyIME/scripts/native_event_smoke.py
python3 src/unifyIME/scripts/native_event_smoke.py --strategy-only
UNIFYIME_STRATEGY_ROUND=3 python3 src/unifyIME/scripts/native_event_smoke.py --strategy-only
```

完整執行包含 62 項策略檢查與 394 個原生事件案例；`--strategy-only` 只跑策略，`UNIFYIME_STRATEGY_ROUND` 可選 1 至 6，第 6 組為省略聲調補救。產物路徑由程式印出，可用 `--output-dir` 指定。測試建立獨立資料及偏好 suite，並停用 Core ML 模型；準確率結果須在相同資料及模型設定下比較。本機產物、個人資料與未發布的研究工具不隨 GitHub 同步。
