...
| 紀錄 | 範圍 |
| --- | --- |
| [五輪組字選字修正](組字選字五輪檢查與修正_20260921.md) | 最新整體結果、游標前進、重選鎖定、詞頻保存、快取隔離與詞內插入 |
...
完整執行包含 30 項策略檢查與 316 個原生事件案例；`--strategy-only` 只跑策略，`UNIFYIME_STRATEGY_ROUND` 可選 1 至 5。產物路徑由程式印出，可用 `--output-dir` 指定。測試建立獨立資料及偏好 suite，並停用 Core ML 模型；準確率結果須在相同資料及模型設定下比較。本機產物、個人資料與未發布的研究工具不隨 GitHub 同步。
