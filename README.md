...
輸入設定底部提供預設開啟的「Shift 切換中英文」。單獨按下並放開 Shift，可在本輸入法的中英混打與英文直輸之間切換，不切換到其他輸入法。切換後會保持模式，放開 Shift、繼續打字或切換視窗都不切回，直到下一次單按 Shift（或關閉此功能）。切換時會在輸入游標旁短暫顯示「中」或同風格的「A」，繼續打字後隱藏；系統輸入來源維持不變。切換前會送出目前組字；Shift 搭配字母、數字或其他快捷鍵不切換模式。關閉此功能後回到中英混打。各設定的說明在滑鼠移入或鍵盤聚焦時立即顯示。
