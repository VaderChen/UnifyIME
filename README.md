...
可在偏好設定的「關於」頁按「版本更新」，檢查 GitHub 最新正式 Release。若有新版，確認版本資訊並按「下載並安裝」後，程式會下載符合目前架構的 DMG，顯示下載進度百分比與已下載大小，完成驗證後自動啟動安裝程式；選擇「稍後」不會下載或安裝。沒有新版、網路失敗或缺少相符安裝檔時，介面會顯示狀態。下載完成後會切換為驗證與準備安裝狀態；確認及錯誤對話框顯示全一輸入法圖示。若舊版內建更新失敗，可從 Releases 手動下載 DMG 安裝新版。
